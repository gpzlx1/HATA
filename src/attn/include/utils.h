...
template <typename KernelTraits, typename TiledCopy, typename Engine,
          typename Layout, typename ElemType, typename IndexType>
__forceinline__ __device__ void gather_g2s_copy(
    TiledCopy tiled_copy, const Flash_fwd_params &params,
    Tensor<Engine, Layout> &tDsD, ElemType *__restrict__ S_ptr,
    IndexType const *__restrict__ idx_ptr, const int row_stride,
    const int block, const int tid, const int max_len = 0) {
  static constexpr int kNThreads = KernelTraits::kNThreads;
  static constexpr int kHeadDim = KernelTraits::kHeadDim;
  static constexpr int kBlockN = KernelTraits::kBlockN;
  static constexpr int kBlockKSmem = KernelTraits::kBlockKSmem;
  static constexpr int kNumElemSmemUnit = 8;

  static constexpr int kNumElemThrBlock = kNThreads * kNumElemSmemUnit;
  static constexpr int kNumRowThrBlock = kNumElemThrBlock / kBlockKSmem;
  static constexpr int kNumElem = kBlockN * kHeadDim;
  static constexpr int kNumStep = kNumElem / kNumElemThrBlock;
  static constexpr int kNumGroup = kHeadDim / kBlockKSmem;

  for (int step = 0; step < kNumStep; step += 1) {
    int group_step = step % kNumGroup;
    int seq_step = step / kNumGroup;
    int t_offset = tid * kNumElemSmemUnit;
    int dim_idx = t_offset % kBlockKSmem + group_step * kBlockKSmem;
    int token_idx = t_offset / kBlockKSmem + seq_step * kNumRowThrBlock;

    if (max_len && token_idx >= max_len) {
      cute::clear(tDsD(_, seq_step, group_step));
    }

    else {
      int fetch_idx = idx_ptr[block * kBlockN + token_idx];
      auto s_tensor =
          make_tensor(make_gmem_ptr(S_ptr + fetch_idx * row_stride + dim_idx),
                      Shape<Int<kNumElemSmemUnit>>{});
      cute::copy(tiled_copy, s_tensor, tDsD(_, seq_step, group_step));
    }
  }
}
