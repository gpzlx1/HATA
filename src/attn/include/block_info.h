...
struct BlockInfo {
  ...
  __forceinline__ __device__ index_t k_offset(const index_t batch_stride,
    ...
    return bidb * batch_stride + leftpad_k * row_stride + bidh * head_stride;
  }

  template <typename index_t>
  __forceinline__ __device__ index_t idx_offset(const index_t batch_stride,
                                                const index_t head_stride,
                                                const int bidb,
                                                const int bidh) const {
  ...
  const int actual_seqlen_gather;
};
