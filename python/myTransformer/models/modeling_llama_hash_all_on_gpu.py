        ...
        if is_prefill:
            ...
            if self.layer_idx >= past_key_value.get_num_skip_layers():
                ...
                encoded_query = past_key_value.decode_encode_hash(
                    query_states, kvcache_len - 1, self.layer_idx)
                torch.cuda.nvtx.range_pop()
                ...
                attn_output, attn_lse = KVLib.flash_index_decode(
                ...
                torch.cuda.nvtx.range_push("middle attention")
                attn_output, attn_lse = KVLib.flash_decode(
                    query_states, key_states, value_states, self.sacle,
                    kvcache_len)
                torch.cuda.nvtx.range_pop()
