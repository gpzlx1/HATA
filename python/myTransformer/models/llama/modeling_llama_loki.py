...
class CustomLlamaAttention(LlamaFlashAttention2):
    ...
    def forward(
        ...
        past_key_value: Optional[LokiStaticCache] = None,
        ...
        **kwargs,
        ...
        if self.layer_idx >= past_key_value.get_num_skip_layers():
            if is_prefill:
                ...
                torch.cuda.nvtx.range_push("apply pca")
                query_states = past_key_value.encode_query(
                    query_states, self.layer_idx)
                key_states = past_key_value.encode_key(key_states,
                                                       self.layer_idx)
                torch.cuda.nvtx.range_pop()
                torch.cuda.nvtx.range_push("append kcache")
...
class CustomLlamaModel(LlamaModel):
    ...
    def forward(
        ...
        attention_mask: Optional[torch.Tensor] = None,
        position_ids: Optional[torch.LongTensor] = None,
        past_key_values: Optional[LokiStaticCache] = None,
