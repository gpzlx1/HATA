    ...
    def forward(
        ...
        if self.layer_idx >= past_key_value.get_num_skip_layers():
            if is_prefill:
                ...
                prev_query = self.query_key_value(prev_query)
                if self.multi_query_attention:
                    (prev_query, _, _) = prev_query.split(
                        [
                            self.num_attention_heads_per_partition *
                            self.hidden_size_per_attention_head,
                            self.num_multi_query_groups_per_partition *
                    ...
                    prev_query = prev_query.view(prev_query.size()[:-1] + (
                        self.num_attention_heads_per_partition,
                        self.hidden_size_per_attention_head))
                else:
                    new_tensor_shape = prev_query.size()[:-1] + \
                                    (self.num_attention_heads_per_partition,
                                        3 * self.hidden_size_per_attention_head)
                    prev_query = prev_query.view(*new_tensor_shape)

                    # [b, sq, np, 3 * hn] --> 3 [b, sq, np, hn]
                    (prev_query, _,
                     _) = split_tensor_along_last_dim(prev_query, 3)
                prev_query = prev_query.view(
...
class CustomGLMBlock(GLMBlock):
    ...
    def forward(
        ...
        attention_mask,
        position_ids,
        kv_cache: Optional[InfiniGenStaticCache] = None,
        use_cache=True,
    ):
        # hidden_states: [s, b, h]
        ...
        if kv_cache.get_cur_q_len() == 1:
            layer_idx = self.self_attention.layer_idx
