...
class RotaryEmbedding(nn.Module):
    ...
    def forward_impl(self,
        ...
        theta = 1.0 / (base**(torch.arange(
            0, n_elem, 2, dtype=torch.float, device=device) / n_elem))
...
class SdpaAttention(CoreAttention):
    ...
    def forward(self, query_layer, key_layer, value_layer, attention_mask):
        if attention_mask is None and query_layer.shape[2] == key_layer.shape[
                2]:
...
class SelfAttention(torch.nn.Module):
    ...
    def __init__(self, config: ChatGLMConfig, layer_number, device=None):
        ...
        self.core_attention = CORE_ATTENTION_CLASSES[
            config._attn_implementation](config, self.layer_number)
...
class ChatGLMPreTrainedModel(PreTrainedModel):
    ...
    def get_masks(self, input_ids, past_key_values, padding_mask=None):
        ...
        if past_length:
            full_attention_mask = torch.cat((torch.ones(
...
class ChatGLMForSequenceClassification(ChatGLMPreTrainedModel):
    ...
    def __init__(self, config: ChatGLMConfig, empty_init=True, device=None):
        ...
        self.classifier_head = nn.Linear(config.hidden_size,
                                         config.num_labels,
                                         bias=True,
                                         dtype=config.torch_dtype)
