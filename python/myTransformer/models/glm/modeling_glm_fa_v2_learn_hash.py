...
class HashTrainer(torch.nn.Module):
    ...
    def loss_func(self,
        ...
        eye = torch.eye(self.hash_weight.shape[1], device=q.device)
        similarity = (q[:, None, :] @ k[:, :, None] / math.sqrt(self.head_dim))
        similarity = torch.exp(similarity - similarity.max()).view(-1)
        similarity_loss = torch.norm(q_hash - k_hash, dim=-1)
        similarity_loss = torch.pow(similarity_loss,
                                    2).view(-1) / math.sqrt(num_items)
        similarity_loss = epsilon_ * torch.sum(
            similarity * similarity_loss).view(-1) / num_items
        balance_loss = torch.sum(q_hash + k_hash,
                                 dim=0) / num_items / self.rbit
        balance_loss = lambda_ * (
            balance_loss[None, :] @ balance_loss[:, None]).view(-1)
        decorrelattion_loss = eta_ * torch.norm(
            self.hash_weight.transpose(-1, -2) @ self.hash_weight -
            eye).view(-1)
    ...
    def train_for_one_epoch(self, query, key):
        _, _, num_head, head_dim = query.shape
        _, seq_len, num_kv_head, head_dim = key.shape
        gqa_size = num_head // num_kv_head

        seq_len -= self.batch_size  # for causal

        query = query[:, -self.batch_size:, :, :].detach().to(self.dtype)
        key = key[:, :seq_len, :, :].detach().to(self.dtype)

        max_iters = min(self.training_iter,
                        (seq_len + self.batch_size - 1) // self.batch_size)

        # repeat key
        key = key.view(-1, num_kv_head, 1, head_dim)
        key = key.expand(-1, -1, gqa_size, -1)
        query = query.view(-1, num_kv_head, gqa_size, head_dim)
        num_keys = key.shape[0]

        with torch.enable_grad():
            for e in range(self.training_epoch):
                rand_idx = torch.randperm(num_keys, device=key.device)
                key = key[rand_idx, :, :, :]
                for i in range(max_iters):
                    self.train()
                    batch_key = key[i * self.batch_size:(i + 1) *
                                    self.batch_size, :, :, :]
                    batch_query = query

                    hash_weight = self.hash_weight.unsqueeze(0)

                    batch_query_code = (batch_query @ hash_weight)
                    batch_key_code = (batch_key @ hash_weight)

                    batch_query = batch_query.view(-1, self.head_dim)
                    batch_key = batch_key.view(-1, self.head_dim)
                    batch_query_code = batch_query_code.view(-1, self.rbit)
                    batch_key_code = batch_key_code.view(-1, self.rbit)
                    loss, similarity_loss, balance_loss, decorrelattion_loss = self.loss_func(
                        batch_query, batch_key, batch_query_code,
                        batch_key_code)
