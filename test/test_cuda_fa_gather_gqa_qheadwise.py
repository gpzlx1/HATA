...
def gather_sdpa(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor,
    ...
    gqa_size = q.shape[1] // k.shape[1]
    k = repeat_kv(k, gqa_size)
    v = repeat_kv(v, gqa_size)
...
def gather_kv(k: torch.Tensor, v: torch.Tensor, idx: torch.Tensor):
    ...
    k = k.transpose(1, 2)
    v = v.transpose(1, 2)
    gqa_size = idx.shape[1] // k.shape[1]
    k = repeat_kv(k, gqa_size)
    v = repeat_kv(v, gqa_size)
    idx = idx.unsqueeze(-1).expand(-1, -1, -1, d)
    gather_k = torch.gather(k, -2, idx)
...
for gather_len in gather_lens:
    gather_idx = torch.randperm(
        ...
        dtype=torch.int32,
        device=torch.device("cuda"))[:batch_size * num_heads * gather_len]
    gather_idx = gather_idx.view(batch_size, num_heads, gather_len) % seq_len
    gather_idx_long = gather_idx.long()
    print("\ngather_len: ", gather_len, gather_idx.shape)
