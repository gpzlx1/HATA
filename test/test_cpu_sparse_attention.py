...
def test_cpu_attention():
    ...
    for seq in [100000]:
        ...
        cpu_query = capi.create_tensor([bsz, num_head, 1, head_dim], 32).reshape(bsz, num_head, 1, head_dim)
        cpu_key = capi.create_tensor([bsz, num_head, seq, head_dim], 16).reshape(bsz, num_head, seq, head_dim)
        cpu_value = capi.create_tensor([bsz, num_head, seq, head_dim], 16).reshape(bsz, num_head, seq, head_dim)
        ...
        for _ in range(30):
            ...
            tic = time.time()
            result = cpu_attention.SparseAttention(cpu_query, cpu_key, cpu_value,
